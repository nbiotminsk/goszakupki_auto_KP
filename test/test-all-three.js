...
async function testAllThreePages() {
  let browser = null;
  try {
    console.log("🚀 Запуск браузера для тестирования всех трёх страниц...");
    ...
    const pages = [
      {
        ...
        expected: "1 ед.",
        description: '"1 условная единица(усл. ед.), 2 243.00 BYN"',
      },
      {
        name: "Single-source 1",
        url: "https://goszakupki.by/single-source/view/3028316",
        expected: "22 ед.",
        description: '"1 единица(ед.), 22 581.36 BYN"',
      },
      {
        name: "Single-source 2",
        url: "https://goszakupki.by/single-source/view/3028754",
        expected: "12 ед.",
        description: '"12 месяц(мес), 9 576.00 BYN"',
      },
    ];

    console.log("\n" + "=".repeat(80));
    console.log("📄 ПАРСИНГ ВСЕХ ТРЁХ СТРАНИЦ");
    ...
    for (const page of pages) {
      console.log(`\n--- ${page.name} (${page.url}) ---`);
      console.log(`Текст на сайте: ${page.description}`);
    ...
    results.forEach((result, index) => {
      ...
      console.log(`Текст на сайте: ${result.siteText}`);
...
// Запуск теста
testAllThreePages().catch(console.error);
